      ...
      <nav className="w-11/12 px-4 absolute top-10 left-1/2 transform -translate-x-1/2 bg-gray-900 z-50 border-white border-2 rounded-xl">
        <div className="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
          ...
          <ul className="hidden md:flex gap-6 text-white font-medium">
            {links.map(link => (
              <li key={link} className="hover:text-blue-300 cursor-pointer transition">
                {link}
              </li>
            ))}
          </ul>
        ...
        {isOpen && (
          <ul className="md:hidden px-4 pb-4 shadow-md">
            {links.map(link => (
              <li key={link} className="py-2 text-white border-b border-white hover:text-blue-300 cursor-pointer transition">
                {link}
              </li>
            ))}
          </ul>
